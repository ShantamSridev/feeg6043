...
def extract_data_from_topics(data_by_topic, aruco_path=None):
    ...
    With ArUco data from CSV file and highlighting matching points.
    ...
    Returns:
        tuple: (extracted_data, simulation_flag, matching_indices)
    """
    extracted_data = {}
    matching_indices = []  # To store indices where ArUco appears in both log and CSV
    
    # Check if this is simulation or real robot data
    simulation_flag = '/groundtruth' in data_by_topic
    
    # Extract data for /est_pose
    ...
    # Extract ArUco data from logs (if available)
    ...
    if simulation_flag and '/groundtruth' in data_by_topic:
        ...
        # Real robot - use ArUco CSV as ground truth
        if 'est_pose' in extracted_data:
            ...
            # Extract columns for ground truth
            ...
            # Find matching points between ArUco in logs and CSV
            if 'aruco' in extracted_data:
                # Time tolerance for matching (in seconds)
                time_tolerance = 0.05
                
                # For each ArUco timestamp in logs, find if there's a match in CSV
                aruco_log_timestamps = extracted_data['aruco']['timestamps']
                for i, log_ts in enumerate(aruco_log_timestamps):
                    # Find closest timestamp in CSV data
                    closest_idx = None
                    min_diff = float('inf')
                    
                    for j, csv_ts in enumerate(groundtruth_timestamps):
                        diff = abs(log_ts - csv_ts)
                        if diff < min_diff:
                            min_diff = diff
                            closest_idx = j
                    
                    # If the closest timestamp is within tolerance, consider it a match
                    if min_diff <= time_tolerance:
                        matching_indices.append((i, closest_idx))
    
    return extracted_data, simulation_flag, matching_indices
...
def plot_trajectory(data, reference_topic, loops=None, highlight_loops=None, output_path=None, matching_indices=None):
    """
    Plot the robot's trajectory, optionally highlighting specific loops and matching ArUco points.
    ...
    Args:
        ...
        matching_indices (list, optional): List of tuples (aruco_log_idx, aruco_csv_idx) for matching points
    """
    ...
    # Highlight matching ArUco points if available
    if matching_indices and 'aruco' in data and 'groundtruth' in data:
        ...
        matching_aruco_x = []
        matching_aruco_y = []
        matching_groundtruth_x = []
        matching_groundtruth_y = []
        
        for aruco_idx, gt_idx in matching_indices:
            if aruco_idx < len(aruco_data['eastings']) and gt_idx < len(groundtruth_data['eastings']):
                matching_aruco_x.append(aruco_data['eastings'][aruco_idx])
                matching_aruco_y.append(aruco_data['northings'][aruco_idx])
                matching_groundtruth_x.append(groundtruth_data['eastings'][gt_idx])
                matching_groundtruth_y.append(groundtruth_data['northings'][gt_idx])
        
        # Plot matching points
        plt.scatter(matching_aruco_x, matching_aruco_y, color='red', marker='o', s=100, 
                   label='Matching ArUco (Log)', alpha=0.5)
        plt.scatter(matching_groundtruth_x, matching_groundtruth_y, color='purple', marker='o', s=100, 
                   label='Matching ArUco (CSV)', alpha=0.5)
        
        # Connect matching points with lines
        for i in range(len(matching_aruco_x)):
            plt.plot([matching_aruco_x[i], matching_groundtruth_x[i]], 
                    [matching_aruco_y[i], matching_groundtruth_y[i]], 
                    'k--', alpha=0.3)
...
def main():
    ...
    parser.add_argument('--highlight-matches', action='store_true', help='Highlight matching ArUco points between log and CSV')
    ...
    try:
        ...
        # Extract data from topics with matching ArUco detection
        extracted_data, is_simulation, matching_indices = extract_data_from_topics(data_by_topic, args.aruco_csv)
        ...
        reference_topic = 'groundtruth' if is_simulation else 'groundtruth'  # Using filtered CSV data as groundtruth
        reference_name = "Ground Truth" if is_simulation else "Aruco Markers (CSV)"
        
        print(f"Mode: {'Simulation' if is_simulation else 'Real robot'} (using {reference_name} as reference)")
        print(f"Found {len(matching_indices)} matching ArUco points between log and CSV")
        ...
        # Plot the trajectory with matching points highlighted if requested
        trajectory_output = f"{args.output}{loop_suffix}_trajectory.png" if args.output else None
        if args.highlight_matches:
            plot_trajectory(filtered_data, reference_topic, highlight_loops=highlight_loops, 
                           output_path=trajectory_output, matching_indices=matching_indices)
        else:
            plot_trajectory(filtered_data, reference_topic, highlight_loops=highlight_loops, 
