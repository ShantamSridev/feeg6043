...
from IMR_02_DSTRONG import train,train_sim, predict, predict_sim
...
class LaptopPilot:
    ...
    def __init__(self, simulation):
        ...
        self.prediction = False, False, False, False
        print("training.........")
        self.gpc = train(100) 
        self.gpc_sim = train_sim()   
        print("trained")
    ...
    # This is a callback function that is called whenever a message is received        
        ...
        lidar_data = np.column_stack((msg.ranges, msg.angles))
        if self.simulation == False: 
            self.prediction = predict(lidar_data,self.gpc)
        else:
            self.prediction = predict_sim(lidar_data,self.gpc_sim)
    ...
    def create_training_data(self, env_map, lidar, sigma_observe):
        ...
        for dist in np.arange(0.1, 0.5, 0.2):
            for i in range(40):
                ...
                if i <= 10:  # southwest corner
                    ...
                    p[1] = 2.0 - dist
                    p[2] = np.deg2rad(135)
                print("end of marker 1", "dist =", dist, "i =", i)
                # add random offsets
                ...
                if (
                    ...
                    print("end of marker 2.3", "dist =", dist, "i =", i)
        ...
        for i in range(40):
            ...
            print("End loop 2, length of corner_training =", len(corner_training))
    ...
    def infinite_loop(self):
        ...
        if (self.initialise_pose != True and
            ...
            # print("TRAINING GAUSSIAN MODEL, PLEASE WAIT")
                        ...
                        ### GPC observations ###
            #print(self.prediction)
            if self.prediction[0] == True:
                z_lm = [0,0]
                corner_loc = [0,0]
                corner, id, z_lm[0], z_lm[1] = self.prediction
                #print(z_lm)
                #print(p_robot)
                t_em = self.lidar.rangeangle_to_loc(p_eb, z_lm)
                t_em = t_em.reshape((2,1))
                print("Found corner at:", t_em)
                z_lm = np.array(z_lm).reshape((2,1))
                self.landmark_id += 1
                z_lm, sigma_rtheta, t_lm, sigma_xy = self.lidar.loc_to_rangeangle(p_eb, t_em, sigma_observe = self.sigma_observe)

                t_lm = np.array(t_lm).reshape((2,1))

                self.graph.observation(t2v(H_eb.H@self.H_bl.H@v2t(t_lm)),sigma_xy,self.landmark_id,t_lm) # Task
