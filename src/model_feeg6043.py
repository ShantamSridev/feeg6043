    ...
    def construct_graph(self,visualise_flag = False):        
        ...
        for k in range(self.e):
            ...
            if edge_type == 'motion':
                ...
                # (1) use the *sum* of covariances for a relative‐motion constraint
                
                sigma_ij = self.pose_covariance[j] + self.pose_covariance[i]
                
                # (2) regularise so it’s never singular
                eps = 1e-6
                sigma_ij = sigma_ij + eps * Identity(3)
                
                # now safe to invert
                inv_sigma = Inverse(sigma_ij)
                
                # populate information vector and matrix
                self.b[3*i:3*i+3] += (e_ij.T @ inv_sigma @ A_ij).T
                self.b[3*j:3*j+3] += (e_ij.T @ inv_sigma @ self.bij).T
                
                self.H[3*i:3*i+3,3*i:3*i+3] += A_ij.T @ inv_sigma @ A_ij
                self.H[3*i:3*i+3,3*j:3*j+3] += A_ij.T @ inv_sigma @ self.bij
                self.H[3*j:3*j+3,3*i:3*i+3] += self.bij.T @ inv_sigma @ A_ij
                self.H[3*j:3*j+3,3*j:3*j+3] += self.bij.T @ inv_sigma @ self.bij
