...
class LaptopPilot:
    ...
    def infinite_loop(self):
        ...
        if self.initialise_pose != True and self.measured_wheelrate_right is not None and self.measured_wheelrate_left is not None:  
            ...
            # take current pose estimate and update directly from measurements
            self.est_pose_northings_m = self.measured_pose_northings_m
            self.est_pose_eastings_m = self.measured_pose_eastings_m
            self.est_pose_yaw_rad = self.measured_pose_yaw_rad
            ...
            self.path.wp_progress(self.t, Vector([self.est_pose_northings_m, self.est_pose_eastings_m, self.est_pose_yaw_rad]), self.turning_radius)  # fill turning radius
            ...
            dp[0] = p_ref[0,0] - self.est_pose_northings_m  # Northings difference
            dp[1] = p_ref[1,0] - self.est_pose_eastings_m  # Eastings difference
            dp[2] = p_ref[2,0] - self.est_pose_yaw_rad  # Yaw difference
            dp[2] = (dp[2] + np.pi) % (2 * np.pi) - np.pi  # handle angle wrapping for yaw

            # Transform difference to body frame
            current_pose = Vector([self.est_pose_northings_m, self.est_pose_eastings_m, self.est_pose_yaw_rad])
            H_eb = HomogeneousTransformation(current_pose[0:2], current_pose[2])  # body to earth transform
