...
class LaptopPilot:
    ...
    def __init__(self, simulation):
        ...
        self.v_max = 0.07  # Maximum linear speed (m/s)
        self.w_max = np.deg2rad(31)  # Maximum angular speed (rad/s)
        ...
        self.gpc = joblib.load('src\gaussian_process_classifier.joblib')
    ...
    def infinite_loop(self):
        ...
        if not self.initialise_pose:
            ...
            if  self.update_landmark:
                ...
                if self.last_landmark != self.landmark_index:
                    ...
                    if(self.landmark_index == 1):
                        ...
                        print(f"Observation of Corner {self.landmark_index} at: {t2v(H_eb.H @ self.H_bl.H @ v2t(self.corner_landmark.flatten()))}")
            ...
            if self.constructed_graph == False:
                ...
                true_pose = Vector(3)
                true_pose[0] = self.measured_pose_northings_m
                true_pose[1] = self.measured_pose_eastings_m
                true_pose[2] = self.measured_pose_yaw_rad

                graph_pose = graph_opt.reduce2pose()

                update_pose = next((p for p in reversed(graph_pose.pose) if np.any(p != 0)), None)
                update_pose = np.array(update_pose).flatten()

                print('Graph Optimised')
                print("Pose:", update_pose)
        
                self.state[N] = update_pose[N]
                self.state[E] = update_pose[E]
                self.state[G] = update_pose[G]
#
                #self.state = graph_opt.pose[-1]
                update_covarience= next((p for p in reversed(graph_pose.pose_covariance) if np.any(p != 0)), None)

                self.sigma = update_covarience
